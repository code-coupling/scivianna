...
class PolygonCoords:
    ...
    def __init__(
        ...
        z_coords: Union[List[float], np.ndarray] = None,
    ):
        ...
        z_coords : Union[List[float], np.ndarray]
            Z coordinates of the polygon vertices
        """
        if type(x_coords) not in (list, np.ndarray):
            raise TypeError(
                f"x_coords must be a numpy array or list, found {type(x_coords)}"
            )
        if type(y_coords) not in (list, np.ndarray):
            raise TypeError(
                f"y_coords must be a numpy array or list, found {type(y_coords)}"
            )
        if z_coords is not None and type(z_coords) not in (list, np.ndarray):
            raise TypeError(
                f"z_coords must be a numpy array or list, found {type(z_coords)}"
            )

        if len(x_coords) != len(y_coords):
            raise ValueError(
                f"Given polygons coords must have the same length, found X : {len(x_coords)} and Y : {len(y_coords)}"
            )
        if z_coords is not None and len(z_coords) != len(y_coords):
            raise ValueError(
                f"Given polygons coords must have the same length, found Z : {len(z_coords)} and Y : {len(y_coords)}"
            )
        ...
        """
        self.z_coords: np.ndarray = None if z_coords is None else np.array(z_coords)
        """ Z coordinate of each vertex of a polygon
        """
    ...
    def rotate(self, origin: Tuple[float, float], angle: float):
        ...
        """
        if self.z_coords is not None:
            raise ValueError("rotate function called on a 3D PolygonCoords element, this function is only valid for 2D elements.")
...
class PolygonElement:
    ...
    def __init__(
        self, exterior_polygon: PolygonCoords, holes: List[PolygonCoords], cell_id: str
    ):
        """PolygonCoords object constructor.
        ...
        if not isinstance(exterior_polygon, PolygonCoords):
            raise TypeError(
                f"exterior_polygon expects a PolygonCoords object, found {type(exterior_polygon)}"
            )
        ...
        for i in range(len(holes)):
            if not isinstance(holes[i], PolygonCoords):
                raise TypeError(
                    f"holes expects a list of PolygonCoords objects, found {type(holes[i])} at item {i}"
                )

        if any([e.z_coords is not None for e in holes + [exterior_polygon]]) \
            and not all([e.z_coords is not None for e in holes + [exterior_polygon]]):
            raise ValueError("Some polygons have Z coordinates but not all.")
        ...
        """
        self.is_3d: bool = all([e.z_coords is not None for e in holes + [exterior_polygon]])
        """ The stored polygons have a Z coordinate
        """
    ...
    def to_shapely(self, z_coord: float = None) -> shapely.Polygon:
        ...
        if self.is_3d:
            return shapely.Polygon(
                np.array(
                    [
                        ...
                        self.exterior_polygon.z_coords
                    ]
                ),
                [
                    np.array(
            ...
            if z_coord is None:
                return shapely.Polygon(
                    np.array(
                        [self.exterior_polygon.x_coords, self.exterior_polygon.y_coords]
                    ),
...
class PolygonCell:
    ...
    def __init__(
        self, tris: List[PolygonCoords], cell_id: str
    ):
        """PolygonCoords object constructor.
        ...
        ----------
        tris : PolygonCoords
            List for polygons defining the cell tris
        cell_id : str
            cell, associated to the polygon, id
        """
        if not isinstance(tris, list):
            raise TypeError(f"holes expects a list, found {type(tris)}")

        for i in range(len(tris)):
            if not isinstance(tris[i], PolygonCoords):
                raise TypeError(
                    f"holes expects a list of PolygonCoords objects, found {type(tris[i])} at item {i}"
                )

        if not all([e.z_coords is not None for e in tris]):
            raise ValueError("Some polygons don't have Z coordinates.")

        self.cells: List[PolygonCoords] = tris
        """ Polygon that surrounds a polygonal object
        """
        self.cell_id: str = cell_id
        """ Cell, associated to the polygon, id
    ...
    def translate(self, dx: float, dy: float, dz: float):
        """Translates the PolygonElement by (dx, dy, dz)
...
def numpy_2D_array_to_polygons(
    ...
    if not len(x) == arr.shape[1]:
        raise ValueError(
            f"len(x) must have the same length as arr first coordinate, found {len(x)} and {arr.shape[1]}"
        )
    if not len(y) == arr.shape[0]:
        raise ValueError(
            f"len(y) must have the same length as arr second coordinate, found {len(y)} and {arr.shape[0]}"
        )
    ...
    values, direct, inv = np.unique(
        arr.flatten(), return_index=True, return_inverse=True
    )  # Values found
    ...
    transform1 = Affine.translation(
        x0 - (x1 - x0) / len(x) / 2, y0 - (y1 - y0) / len(y) / 2
    ) * Affine.scale((x1 - x0) / len(x), (y1 - y0) / len(y))
    shape_gen = (
        (shape(s), val)
        for s, val in rasterio.features.shapes(index_arr, transform=transform1)
    )
    ...
    for s, val in shape_gen:
        ...
        polygon_element_list.append(
            PolygonElement(
                exterior_polygon=PolygonCoords(
                    x_coords=np.array([vert[0] for vert in s.exterior.coords]),
                    y_coords=np.array([vert[1] for vert in s.exterior.coords]),
                ),
                holes=[
                    PolygonCoords(
                        x_coords=np.array([vert[0] for vert in interior.coords]),
